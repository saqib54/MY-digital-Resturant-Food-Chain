...
// Remove pork category as requested
let categories = [
    "lamb",
    "goat",
    "beef",
...
// Function to render meal details with PKR prices
function renderMeals(mealData) {
    // Convert USD price to PKR (assuming 1 USD = 275 PKR)
    const priceInPKR = (10.20 * 275).toFixed(2);
    return mealData.map(
        (mealDetails) => `
        <div class="col-sm-6 col-xl-4 item">
            <div class="card">
                <div class="card-thumbnail">
                    ...
                    <button class="btn btn-warning add-to-cart-button" data-product-id="${mealDetails.id}" data-product-name="${mealDetails.name}" data-product-price="${priceInPKR}" data-product-img-source="${mealDetails.imgSrc}">+ Add to Cart</button>
            	</div>
            	<div class="card-body">
...
function filterMenu() {
    ...
    menuItems.forEach((item) => {
        ...
        const itemPrice = parseFloat(
            item.querySelector("p").innerText.replace('₨', '').replace('$', '')
        );
        ...
        switch (priceFilter) {
            case "all":
                ...
                break;
            case "0-5":
                priceMatch = itemPrice >= 0 && itemPrice <= (5 * 275);
                break;
            case "5-10":
                priceMatch = itemPrice > (5 * 275) && itemPrice <= (10 * 275);
                break;
            case "10-15":
                priceMatch = itemPrice > (10 * 275) && itemPrice <= (15 * 275);
                break;
            case "15-20":
                priceMatch = itemPrice > (15 * 275) && itemPrice <= (20 * 275);
                break;
        }
        ...
        if (priceFilter !== 'all') {
            ...
            showByPrice = itemPrice >= (minPrice * 275) && itemPrice <= (maxPrice * 275);
        }
